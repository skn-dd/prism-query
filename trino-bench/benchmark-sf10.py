#!/usr/bin/env python3
"""Prism vs Trino Benchmark at SF10 (60M rows lineitem, 15M rows orders)."""
...
def main():
    ...
    print("=" * 70)
    print("  Prism vs Trino Benchmark — SF10 (60M rows)")
    print("=" * 70)

    # ── Trino Baseline (tpch SF10) ──
    print("\n--- Trino Baseline (tpch.sf10) ---")
    trino_queries = [
    ...
    for name, sql in trino_queries:
        median, rows = run_benchmark(name, "tpch", "sf10", sql)
        results[name] = {"ms": median, "rows": rows}

    # ── Prism (prism.tpch with SF10 data) ──
    print("\n--- Prism (prism.tpch, SF10 data) ---")
    prism_queries = [
    ...
    # ── Comparison ──
    print("\n" + "=" * 70)
    print("  COMPARISON — SF10 (60M lineitem rows)")
    print("=" * 70)
