#!/usr/bin/env python3
"""Prism vs Trino Benchmark at SF100 (600M rows lineitem, 150M rows orders)."""
...
# ── Prism Queries (prefixed columns) ──
...
def main():
    ...
    print("=" * 70)
    print("  Prism vs Trino Benchmark — SF100 (600M rows)")
    print("=" * 70)

    # ── Trino Baseline (tpch SF100) ──
    print("\n--- Trino Baseline (tpch.sf100) ---")
    trino_queries = [
    ...
    for name, sql in trino_queries:
        median, rows = run_benchmark(name, "tpch", "sf100", sql)
        results[name] = {"ms": median, "rows": rows}

    # ── Prism (prism.tpch with SF100 data) ──
    print("\n--- Prism (prism.tpch, SF100 data) ---")
    prism_queries = [
    ...
    # ── Comparison ──
    print("\n" + "=" * 70)
    print("  COMPARISON — SF100 (600M lineitem rows)")
    print("=" * 70)
    ...
    import os
    out = os.path.join(os.path.dirname(os.path.abspath(__file__)), "results-sf100.json")
